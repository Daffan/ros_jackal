            ...
            if len(trajs) >= num_trials:
                for traj in trajs:
                    ...
                    if isinstance(world, str):
                        if world.startswith("habitat"):
                            world = int(world.split("habitat")[-1].split(".")[0])
                        else:
                    ...
                    results[world].append((ep_return, ep_length, success, ep_time, collision))
                j.Remove()
                ...
                print("finishing world %d: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, support %d/%d" \
                    %(world, mean_return, mean_length, mean_success, mean_time, mean_s_time, mean_collision, mean_all_time, mean_metrics, len(times), len(results[world])))
